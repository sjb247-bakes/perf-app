...
export default function IntegrationsPage() {
  ...
  useEffect(() => {
    ...
    if (params.get('success')) {
      setStatus({ type: 'success', message: params.get('success')! })
    } else if (params.get('error')) {
      setStatus({ type: 'error', message: params.get('error')! })
    }
  }, [])
      ...
      <main className="container mx-auto px-4 py-8 max-w-6xl">
        <div className="space-y-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-start">
            {/* Garmin Connect */}
            <Card className="bg-zinc-900 border-zinc-800 text-white overflow-hidden flex flex-col h-full">
              ...
              <CardHeader>
                <CardTitle className="flex items-center justify-between">
                  Connect Garmin
                  {isGarminConnected && (
                    <span className="text-[10px] bg-green-500/20 text-green-400 px-2 py-0.5 rounded-full border border-green-500/50 uppercase tracking-wider font-bold">
                      Connected
                    </span>
                  )}
                </CardTitle>
              ...
              <form onSubmit={handleGarminConnect} className="flex-1 flex flex-col">
                <CardContent className="space-y-4 flex-1">
                  ...
                  <div className="mt-4 flex items-start gap-3 bg-zinc-950 p-4 rounded-lg border border-zinc-800">
                    ...
                    <div className="text-[11px] text-zinc-500 leading-relaxed">
                      Your credentials are encrypted at rest. We only use them to sync your performance data.
                    </div>
                  </div>
                </CardContent>
            ...
            <Card className="bg-zinc-900 border-zinc-800 text-white overflow-hidden flex flex-col h-full">
              ...
              <CardHeader>
                ...
                <CardDescription className="text-zinc-400">
                  Sync your activities to layer training load over your recovery data.
                </CardDescription>
              </CardHeader>
              <CardContent className="flex-1">
                <div className="flex flex-col items-center justify-center py-8 text-center space-y-4">
                  <div className="h-16 w-16 bg-orange-600/10 rounded-full flex items-center justify-center border border-orange-600/20">
                    <Activity className="h-8 w-8 text-orange-600" />
                  </div>
                  <div className="text-sm text-zinc-500 max-w-[240px]">
                    Authorize the app to read your activity history and training load.
                  </div>
                </div>
              </CardContent>
              <CardFooter className="pt-4">
                ...
                >
                  {isStravaConnected ? 'Update Connection' : 'Connect Strava Account'}
                </Button>
              </CardFooter>
            </Card>
          </div>
