...
Deno.serve(async (req) => {
  ...
  console.log("🚀 Starting Daily Sync (Garmin + Strava)...");
  ...
  for (const user_id of users) {
    ...
    if (garmin) {
      try {
        ...
        const [sleep, hrv, wellness] = await Promise.all([
        ...
        console.error(`❌ Garmin error for ${user_id}:`, e.message);
      }
    }
    ...
    if (strava) {
      try {
        ...
        if (refreshRes.ok) {
          ...
          // Fetch activities from last 24h
          const after = Math.floor(Date.now() / 1000) - (24 * 60 * 60);
          const actRes = await fetch(`https://www.strava.com/api/v3/athlete/activities?after=${after}`, {
        ...
        console.error(`❌ Strava error for ${user_id}:`, e.message);
      }
    }
  }
